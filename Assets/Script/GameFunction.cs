...
public class GameFunction : MonoBehaviour {
	...
	void Update () {
		...
		if (isPlaying) {
			...
			if (coinTime > 1f) {
				Vector3 pos = new Vector3 (Random.Range(-9f, 9f), 5f, 0);
				Instantiate (CoinObject, pos, transform.rotation);
				coinTime = 0f;
			}

			if (millileTime1 > 4f) {
				Vector3 pos = new Vector3 (-9f, Random.Range(-5f, 5f), 0);
			...
			if (millileTime2 > 5f) {
				Vector3 pos = new Vector3 (9.5f, Random.Range(-5f, 5f), 0);
				Instantiate (Right_Missile, pos, Quaternion.Euler(0, 0, 180));
				millileTime2 = 1.0f;
			}
			/*
		if (millileTime3 > 6f) {
			Vector3 pos = new Vector3 (Random.Range(-9.5f, 9.5f), 5f, 0);
			Instantiate (Top_Missile, pos, Quaternion.Euler(0, 0, 270));
			millileTime3 = 2.0f;
		}
		*/
		}
